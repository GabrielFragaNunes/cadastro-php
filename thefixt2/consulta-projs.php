...
<body>
  <div class="container">
    <h1 class="jumbotron bg-info">Consulta de Projetos</h1>
    ...
    if(count($array) == 0){
        echo "<h2>Não há projetos no banco!</h2>";
        return;
    }
    ?>
    ...
    if(isset($_POST['filtrar'])){
      ...
      if(!empty($pesquisa)){
        $projDAO = new ProjDAO();
        $array = $projDAO->filtrar($pesquisa,$filtro);
        if(count($array) == 0){
        }
      }else{
        echo "<h3>Sua pesquisa não retornou nenhum projeto!</h3>";
      }
    }
    ?>
      ...
      <table class="table table-striped table-bordered table-hover table-condensed">
        ...
        <tbody>
          ...
          foreach($array as $l){
            echo "<tr>";
              echo "<td>$l->idProj</td>";
              echo "<td>$l->nomeproj</td>";
              echo "<td>$l->responsavel</td>";
              echo "<td>$l->duracao</td>";
              echo "<td>$l->devproj</td>";
              echo "<td>$l->tipoproj</td>";
              echo "<td><a href='consulta-projs.php?id=$l->idProj' class='btn btn-danger'>Excluir</a></td>";
              echo "<td><a href='alterar-proj.php?id=$l->idProj' class='btn btn-warning'>Alterar</a></td>";
            echo "</tr>";
          }
          ?>
        </tbody>
